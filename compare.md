...
| **属性系统** | 组件结构体中定义 | 状态驱动的多状态属性系统（like: basic/hover/pressed/disabled），TOML全局配置，上层同步下层，下层强定义覆盖上层，组件中固定使用`prop`表示，使用`Prop, SlotProp, BasicProp, SlotBasicProp, ApplyMapImpl, ApplySlotMapImpl`等traits增强实现 |
