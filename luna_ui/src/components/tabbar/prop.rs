...
use crate::{
    ...
    components::{
        ...
        traits::{BasicProp, ComponentState, Prop}, view::ViewBasicProp,
    },
    error::Error,
    get_get_mut,
    prop::{
...
impl BasicProp for TabbarBasicProp {
    type State = TabbarState;
...
impl Default for TabbarBasicProp {
    fn default() -> Self {
        TabbarBasicProp::from_state(Theme::default(), TabbarState::Basic)
    }
}

impl TryFrom<(&Value, TabbarState)> for TabbarBasicProp {
    type Error = Error;

    fn try_from((value, state): (&Value, TabbarState)) -> Result<Self, Self::Error> {
...
impl TryFrom<(&Item, TabbarState)> for TabbarBasicProp {
    type Error = Error;

    fn try_from((value, state): (&Item, TabbarState)) -> Result<Self, Self::Error> {
        let inline_table = value.as_inline_table().ok_or(Error::ThemeStyleParse(
            "[components.view.$state] should be an inline table".to_string(),
        ))?;
        (inline_table, state).try_into()
...
impl TryFrom<(&InlineTable, TabbarState)> for TabbarBasicProp {
    type Error = Error;

    fn try_from((inline_table, state): (&InlineTable, TabbarState)) -> Result<Self, Self::Error> {
...
impl From<TabbarBasicProp> for ViewBasicProp {
    fn from(value: TabbarBasicProp) -> Self {
        ViewBasicProp {
            theme: value.theme,
