    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.starts_with("rgba") {
            ...
            let g = parts[1].parse::<u8>().map_err(|_| {
                crate::error::Error::ThemeStyleParse("Invalid green value in RGBA color".to_string())
            })?;
            ...
            let a = parts[3].parse::<f32>().map_err(|_| {
                crate::error::Error::ThemeStyleParse("Invalid alpha value in RGBA color".to_string())
            })?;
            Ok(Rgba { r, g, b, a })
        } else {
            return Err(crate::error::Error::ThemeStyleParse(
